...
from sql_lite import add, user_id, product, list_add, list_take_date, list_take
...
async def start_fync(message: Message):
...
async def shop(call: CallbackQuery):
    ...
    await basketsecond(message=call.message)
...
async def basketsecond(message: Message):
    global info, v
    if len(info) < 5:
        await message.answer('Ваша корзина пуста', reply_markup=keyboard5)
    else:
...
async def basket(message: Message):
    await message.answer('Данная функция еще дорабатывается🤯🤯', )
'''
    l = await list_take_date(info[0])
    print(l)
...
async def take_order(call: CallbackQuery):
    ...
    await call.message.edit_reply_markup(reply_markup=None)
    await basketsecond(message=call.message)
    #await call.message.answer('Добавленно в корзину',reply_markup=keyboard5)
'''
